...
from .models import BattleBall, BattleInstance, GuildBattle, battles
...
class ReadyView(discord.ui.View):
    ...
    async def execute_battle(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        battle_log = "\n".join(gen_battle(simulated_battle))
        ...
        battle_logs_embed.add_field(
            name="**Winner:**",
            value=f"{simulated_battle.winner} - Turn: {simulated_battle.turns}",
            inline=False,
        )
...
class BattleSetupView(discord.ui.View):
    ...
    async def ready_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        from .cog import gen_deck, get_battle_embed, prune_unowned_balls
        ...
        if author_ready and opponent_ready:
            if len(guild_battle.battle.p1_balls) < guild_battle.amount_required or len(guild_battle.battle.p2_balls) < guild_battle.amount_required:
                await interaction.response.send_message(
                    f"Both users must add at least {guild_battle.amount_required} {settings.plural_collectible_name}!",
                    ephemeral=True,
                )
                return
