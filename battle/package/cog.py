...
# HP, ATK
BATTLE_TIMEOUT = 900 # How many seconds until a battle plan times out
# TIME

BATTLE_TIMEOUT_TEXT = (
    f"{BATTLE_TIMEOUT // 3600} hour{'s' if BATTLE_TIMEOUT != 3600 else ''}"
    if BATTLE_TIMEOUT % 3600 == 0
    else f"{BATTLE_TIMEOUT // 60} minute{'s' if BATTLE_TIMEOUT != 60 else ''}"
    if BATTLE_TIMEOUT % 60 == 0
    else f"{BATTLE_TIMEOUT} second{'s' if BATTLE_TIMEOUT != 1 else ''}"
)
...
def gen_battle(battle: BattleInstance):
    ...
    while any(ball for ball in battle.p1_balls if not ball.dead) and any(
        ...
        p1_ball = get_active_ball(battle.p1_balls)
        p2_ball = get_active_ball(battle.p2_balls)

        if p1_ball and p2_ball:
            turn += 1

            attack_dealt = attack(p1_ball, p2_ball)
            yield f"Turn {turn}: {p1_ball.owner}'s {p1_ball.name} has dealt {attack_dealt} damage to {p2_ball.owner}'s {p2_ball.name}"

            if p2_ball.dead:
                yield f"Turn {turn}: {p1_ball.owner}'s {p1_ball.name} has killed {p2_ball.owner}'s {p2_ball.name}"
        ...
        if p1_ball and p2_ball:
            ...
            attack_dealt = attack(p2_ball, p1_ball)
            yield f"Turn {turn}: {p2_ball.owner}'s {p2_ball.name} has dealt {attack_dealt} damage to {p1_ball.owner}'s {p1_ball.name}"

            if p1_ball.dead:
                yield f"Turn {turn}: {p2_ball.owner}'s {p2_ball.name} has killed {p1_ball.owner}'s {p1_ball.name}"
...
class Battle(commands.GroupCog):
        ...
        while True:
            try:
                ...
                for battle in battles[:]:
                    if current_time - battle.created_at > BATTLE_TIMEOUT:
    ...
    async def add_balls(self, interaction: discord.Interaction, guild_battle: GuildBattle, countryballs):
    ...
    async def remove_balls(self, interaction: discord.Interaction, guild_battle: GuildBattle, countryballs):
        user_balls = guild_battle.get_user_balls(interaction.user)
        for countryball in countryballs:
            ball = self.build_battle_ball(interaction, countryball, guild_battle)
    ...
    async def add(
        ...
        async for dupe in self.add_balls(interaction, guild_battle, [countryball]):
            if dupe:
                await interaction.response.send_message(
    ...
    async def remove(
        ...
        async for not_in_battle in self.remove_balls(interaction, guild_battle, [countryball]):
            if not_in_battle:
                await interaction.response.send_message(
